...
The historical direction of influence is worth one sentence, with a correction to the standard story. The French word *faisceau* ("sheaf," "bundle") was used by the Italian school of algebraic geometry in the early twentieth century in the sense of a "pencil" of curves or a system of linear equations — a usage that survives in Italian as *fascio* — but it is not the same object as the modern sheaf. The modern sheaf, as a rule assigning data to open sets with restriction and gluing, was introduced independently by Jean Leray in 1946 in his work on the algebraic topology of singular spaces; the term *faisceau* then became standard, and "sheaf" is its English translation. More substantively, the two structures are related by a precise dictionary: every smooth vector bundle over a manifold has a sheaf of sections, and conversely every locally free sheaf of modules over the sheaf of smooth functions is the sheaf of sections of a unique vector bundle. This dictionary — the Serre–Swan theorem in the algebraic and smooth settings, and Ehresmann's theorem in the topological category — is what allows one to move freely between geometric and cohomological formulations of physical theories.
...
- The **unit tangent bundle** $T^1 M \to M$: for $M = S^n$ this is the principal bundle $SO(n+1)/SO(n-1) \to S^n$, whose fiber $S^{n-1}$ is the unit sphere in $T_x S^n$. It is trivial if and only if $S^n$ is parallelizable. In particular $TS^2$ is not trivial — the hairy ball theorem — so no global nonvanishing vector field exists on $S^2$.
...
> **Theorem (Serre–Swan).** The functor $E \mapsto \Gamma(E)$ is an equivalence between the category of smooth vector bundles over $M$ and the category of locally free sheaves of $\mathcal{O}_M$-modules. (In the algebraic category, the same statement for holomorphic vector bundles and locally free sheaves of holomorphic functions is due to Oka; Ehresmann's 1950 contribution was the systematic formalization of fiber bundles and connections, not this equivalence.)
